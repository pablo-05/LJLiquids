      ...
      do while(error.gt.limit.and.(contador.lt.contmax)) ! Condition for convergence
        ...
        do i=1, N
          ...
          h(i)=(i-1)*step*((fold(i)+1.0)*exp(-beta*PHI((i-1)*step))-1.0)
      ...
      do i=1, N
        g(i) = exp( -beta * PHI((i-1)*step) ) * ( fold(i) + 1.0 )
      end do
      ...
      do i=1, N
        ! Calculate r*h(r) — sinft needs r*f(r) for the 3D radial transform
        h(i) = 0.0 ! (i-1)*step*((fold(i)+1.0)*exp(-beta*PHI((i-1)*step))-1.0)
        c(i) = 0.0
        fold(i) = 0.0
      end do
      ...
      if (r .lt. 0.001) then
        PHI = 1.0e30 ! Effectively infinite repulsion
      else
